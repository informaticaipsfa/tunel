...
func (wc *WCredito) RelacionActiva(w http.ResponseWriter, r *http.Request) {
	...
	j, e := wCred.RelacionActiva(wc.Fecha, wc.Desde, wc.Hasta, wc.Estatus)
	if e != nil {
		w.WriteHeader(http.StatusForbidden)
		w.Write([]byte("Error al consultar los datos"))
		return
...
func (wc *WCredito) RelacionPagados(w http.ResponseWriter, r *http.Request) {
	...
	j, e := wCred.RelacionPagados(wc.Fecha, wc.Desde, wc.Hasta, wc.Estatus)
	if e != nil {
		w.WriteHeader(http.StatusForbidden)
		w.Write([]byte("Error al consultar los datos"))
		return
...
func (wc *WCredito) Pagar(w http.ResponseWriter, r *http.Request) {
	...
	wCredito.Pagar(wLiquidar, UsuarioConectado.Login)
	M.Mensaje = wLiquidar.Credito
	M.Tipo = 1
	w.WriteHeader(http.StatusOK)
	j, _ := json.Marshal(M)
